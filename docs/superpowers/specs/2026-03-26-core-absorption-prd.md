...
> Discussion: plugins/core.ts의 commands+entities를 axis/로 이동하여 L4→L5 역참조 제거. APG 파일을 pattern/examples/로 분리.
...
- **Decision**: core.ts 442줄을 4개 axis 파일(navigate, select, expand, value)에 관심사별 분배. composePattern이 middleware도 합성. core() 함수 삭제. APG 파일을 pattern/examples/로 이동. 기각된 대안: (1) ctx 제거하고 axis가 Command 직접 반환 — 네비게이션 알고리즘 중복, 변경 범위 과대. (2) 관심사별 폴더 재구성(focus/, selection/) — 레이어 경계 소멸, APG 패턴 검증 어려움.
...
| S5 | APG 파일이 pattern/examples/에 위치 | UI가 listbox behavior import | `../pattern/examples/listbox`에서 import | |
...
| pattern/examples/ 디렉토리 | 18개 APG 파일 이동 (listbox, tree, treegrid, grid, menu, tabs, accordion, dialog, alertdialog, combobox, radiogroup, toolbar, switch, slider, spinbutton, disclosure) | |
...
| pattern/examples/ 소비자 수정 | `../pattern/listbox` → `../pattern/examples/listbox` 등 (UI 23개 + 테스트 35개) | |
...
| import { listbox } from 'pattern/listbox' | pattern/ 직접 | pattern/examples/에서 import | APG는 composePattern의 사용 예시이지 본문이 아님 | import 경로 변경 | |
...
| pattern/examples/ 내 파일이 상대경로 참조 | listbox.ts: `import { composePattern } from './composePattern'` | examples/로 이동 후 `../composePattern` | 한 단계 위로 | import 경로 수정 | |
...
| 3 | Pattern=APG 검증용, 외부 소비 아님 (feedback_pattern_apg_only) | ② pattern/examples/ | ✅ 준수 — examples/로 분리하여 "본문 vs 예시" 명확화 | — | |
...
| V5 | ①S5 | pattern/examples/에서 APG import | `import { listbox } from '../pattern/examples/listbox'` 성공 | |
