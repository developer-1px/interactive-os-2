...
| 항목 | 역PRD |
|------|-------|
| `NodeState.renaming: boolean` — `__rename__` 엔티티에서 해당 노드가 rename 중인지 파생 | `hooks/useAriaView.ts::getNodeState` |

### 3-2. useAria 변경

| 항목 | 역PRD |
|------|-------|
| `getNodeState()` — `__rename__` 엔티티 조회하여 `renaming` 필드 반영 | `hooks/useAriaView.ts::getNodeState` |
| `getNodeProps().onKeyDown` — event.target이 editable element면 early return (범용 가드) | `hooks/keymapHelpers.ts::isEditableElement`, `hooks/useAriaView.ts::getNodeProps` |

### 3-3. `<Aria.Editable>` 컴포넌트

| 항목 | 역PRD |
|------|-------|
| AriaItemContext에서 nodeId, renaming 상태를 읽음 | `components/aria.tsx::AriaEditable` |
| `field` prop으로 편집 대상 필드 지정 | `components/aria.tsx::AriaEditable` |
| renaming=false: children을 그대로 렌더 | `components/aria.tsx::AriaEditable` |
| renaming=true: contenteditable div로 전환, 텍스트 전체 선택, Enter/Esc 핸들링 | `components/aria.tsx::AriaEditable` |
| confirm 시 el.textContent로 값 읽어서 confirmRename dispatch | `plugins/rename.ts::renameCommands` |
| cancel 시 원래 텍스트 복원 후 cancelRename dispatch | `plugins/rename.ts::renameCommands` |

### 3-4. 데모 페이지 수정

| 항목 | 역PRD |
|------|-------|
| PageRename.tsx — renderItem에 `<Aria.Editable field="label">` 적용 | `pages/PageRename.tsx` |
...
| # | 시나리오 | 예상 결과 | 역PRD |
|---|---------|----------|-------|
| 1 | F2 → 텍스트 타이핑 → Enter | store에 새 값 반영, renaming=false | `__tests__/rename-ui.test.tsx::"F2 → type → Enter confirms rename"` |
| 2 | F2 → 텍스트 타이핑 → Escape | 원래 값 유지, renaming=false | `__tests__/rename-ui.test.tsx::"F2 → type → Escape cancels rename"` |
| 3 | F2 → Enter → Cmd+Z | undo로 이전 값 복원 | `__tests__/rename-ui.test.tsx::"confirmRename undo restores original value (engine level)"` |
| 4 | F2 → ↑↓ 키 입력 | 포커스 이동 안 됨 (커서만 이동) | `__tests__/rename-ui.test.tsx::"arrow keys do not move focus during rename"` |
| 5 | F2 → "abc" 타이핑 | behavior keyMap에 잡히지 않고 텍스트 입력됨 | `__tests__/rename-ui.test.tsx::"arrow keys do not move focus during rename"` |
| 6 | rename 플러그인 없이 Aria.Editable 사용 | 에러 없이 일반 텍스트 렌더 | `__tests__/rename-ui.test.tsx::"Aria.Editable without rename plugin renders children normally"` |
| 7 | NodeState.renaming이 true인 노드 확인 | `__rename__` active:true + 해당 nodeId 일치 시에만 true | `__tests__/rename-ui.test.tsx::"is false by default"`, `"becomes true after F2"` |
| 8 | IME 한글 입력 후 Enter | 조합 완료 후 confirm, 중간 상태에서 confirm 안 됨 | `__tests__/rename-ui.test.tsx::"Enter during IME composition does not confirm"` |
| 9 | F2 → 텍스트 전부 지움 → Enter | 빈 문자열이므로 별도 리액션 없이 원래 값 복원 (cancel과 동일) | `__tests__/rename-ui.test.tsx::"empty string confirms as cancel (restores original)"` |
| 10 | 더블클릭으로 rename 진입 → Enter | F2 진입과 동일하게 동작 | `__tests__/rename-ui.test.tsx::"double-click starts rename"` |
| 11 | rename 중 다른 노드 클릭 | 현재 값으로 confirm 후 클릭된 노드로 포커스 이동 | `__tests__/rename-ui.test.tsx::"blur confirms rename"` |
