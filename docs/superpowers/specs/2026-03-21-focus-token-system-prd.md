...
|---|-------|--------------|------|------|-------------|-------|
| M1 | 키보드로 리스트 탐색 중, 마우스가 다른 아이템 위에 정지 | focused 아이템: `--accent-dim`(bg), hovered 아이템: `--bg-hover`(bg) | 두 아이템이 동시에 하이라이트 | 키보드 포커스 활성 시 hover 스타일 억제. focused 아이템만 하이라이트 | focused 아이템: `--bg-focus`(bg), hovered 아이템: transparent | |
| M2 | focused 아이템에서 Enter/Space로 activate | focused: `--accent-dim` bg | press 중 | press 피드백이 보여야 함. 현재는 focused와 동일하여 구분 불가 | active: `--bg-press`(indigo 0.18) bg — focused보다 한 단계 강함 | |
| M3 | 여러 아이템 선택 후 키보드로 포커스 이동 | selected 아이템: `--bg-select`(green), focused+selected: `--accent-dim`(indigo) | focused가 selected 위에 오면 | selected 배경(green) 유지 + focus를 outline으로 표현. 현재는 focus가 selected를 덮어씀 | focused+selected: `--bg-select`(bg) + `outline: 1.5px solid var(--accent)`(ring) | |
| M4 | `--accent-dim`이 focus와 장식 용도로 혼용 | focus 6곳 + 장식 5곳 모두 `--accent-dim` | focus 토큰 변경 시 | 장식까지 영향받음. 용도별 토큰 분리 필요 | focus → `--bg-focus`, 장식 → `--accent-dim` 유지 | |
...
| `tokens.css` — `[data-theme="light"]` 내 `--bg-press` | 라이트 테마용 press 토큰 값 | |
| `components.css` — focus 룰 | `:focus`, `[data-focused]`의 배경을 `--accent-dim` → `--bg-focus`로 교체 | |
| `components.css` — active 룰 신규 | `:active` 상태에 `--bg-press` 배경 적용 | |
| `components.css` — selected+focused 복합 룰 | `[aria-selected="true"]:focus` 등 → 배경 `--bg-select` 유지 + outline 추가 | |
| `components.css` — hover 억제 룰 | `[data-aria-container]:focus-within` 내 hover 스타일 비활성화 | |
| `kanban.css` — `--accent-dim` → `--bg-focus` 교체 | 3곳 (`:focus` 관련) | |
| `PageVisualCms.css` — focus/select 토큰 교체 | `.cms-step--focused` → `--bg-focus`, `[aria-selected]` → `--bg-select` | |
| `combobox.css` — `.combo-item--focused` → ARIA 방식 | className 기반 → `[data-focused]` 셀렉터로 전환 | |
| `spinbutton.css`, `slider.css` — `--focus-ring` 정리 | fallback chain 제거, 글로벌 outline 시스템 활용 | |
...
| Tab (탭 컴포넌트) focused+selected | tab: selected + focused | Tab 예외: border-bottom 방식 유지, outline 추가하지 않음. selected/focused 구분이 accent border로 이미 처리됨 | tab: accent border 유지 | |
...
| # | 하면 안 되는 것 | 이유 | 역PRD |
|---|---------------|------|-------|
| F1 | `--accent-dim` 토큰 자체를 삭제하거나 값을 변경 | 장식 용도(badge, 인용문, CMS UI)에서 사용 중. focus 분리가 목적이지 accent 스케일 변경이 아님 | |
| F2 | hover 억제를 `[data-aria-container]` 밖으로 확장 | 일반 UI(버튼, 링크, CMS 도구)의 hover는 정상 동작해야 함 | |
| F3 | selected+focused에서 selected 배경을 제거하고 focus 배경으로 대체 | "내가 뭘 선택했는지" 정보 소실 — 현재 버그의 원인이 바로 이것 | |
| F4 | 컴포넌트별 CSS에서 독자적 focus 토큰/값 발명 | `--focus-ring`, `--color-accent` 같은 미정의 변수 참조는 유지보수 불가 | |
| F5 | hover와 selected가 시각적으로 동일해지는 토큰 값 설정 | "선택된 것"과 "마우스가 올라간 것"은 완전히 다른 상태 | |
| F6 | Tab 컴포넌트에 outline 기반 focus 표현 적용 | Tab은 border-bottom 방식이 확정된 예외. outline과 border가 겹치면 시각 노이즈 | |
