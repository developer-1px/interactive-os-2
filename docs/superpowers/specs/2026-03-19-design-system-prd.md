...
| # | 시나리오 | 예상 결과 | 우선순위 |
|---|---------|----------|---------|
| V1 | ListBox 데모 페이지 — 포커스 이동 | `var(--bg-focus)` 배경 표시 | P0 |
| V2 | ListBox 데모 페이지 — 아이템 선택 | `var(--bg-select)` 배경 표시 | P0 |
| V3 | 테마 토글 (dark↔light) | 모든 전환된 컴포넌트가 일관 전환 | P0 |
| V4 | 기존 데모 페이지 시각적 regression | 마이그레이션 전 컴포넌트는 기존과 동일 | P0 |
| V5 | TreeView — 인덴트 레벨 | aria-level별 padding 적용 | P0 |
| V6 | Combobox — activedescendant 포커스 | `[data-focused]` 셀렉터로 하이라이트 | P0 |
| V7 | CSS specificity — inline style 제거된 컴포넌트 | CSS 오버라이드가 정상 작동 | P0 |
