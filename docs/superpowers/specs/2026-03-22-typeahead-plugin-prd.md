...
| 산출물 | 설명 | 역PRD |
|--------|------|-------|
| `typeahead.ts` | `src/interactive-os/plugins/typeahead.ts` — typeahead plugin. module-level 버퍼 + 타이머. `getLabel` 옵션으로 entity에서 텍스트 추출 | |
| Plugin.onUnhandledKey | Plugin 인터페이스에 optional `onUnhandledKey` 추가. keyMap에 매칭되지 않은 키 이벤트를 처리하는 fallback 핸들러 | |
| useAria onKeyDown 확장 | keyMap 매칭 실패 시 plugin.onUnhandledKey 체인을 호출하는 fallback 경로 추가 | |
...
|---|----------------------|---------|----------|-------|
| V1 | M1 | listbox에서 "b" 입력 | "B"로 시작하는 첫 항목으로 포커스 이동 | |
| V2 | M2 | "b" 입력 후 300ms 이내 "a" 입력 | "ba"로 시작하는 항목으로 포커스 (multi-char) | |
| V3 | M3 | "b" 입력 → 600ms 대기 → "b" 입력 | 다음 "B" 항목으로 순환 이동 | |
| V4 | M4 | rename 모드에서 문자 입력 | typeahead 미동작, rename에 문자 입력됨 | |
| V5 | M5 | "z" 입력 (매칭 항목 없음) | 포커스 변화 없음 | |
| V6 | E1 | 빈 리스트에서 문자 입력 | no-op | |
| V7 | E4 | "b" 입력 (소문자) → "Banana" 항목 존재 | 대소문자 무시하고 매칭 | |
| V8 | E5 | IME로 한글 조합 중 | typeahead 미동작 | |
| V9 | E9 | 리스트 끝에서 같은 문자 반복 | 리스트 처음으로 순환 | |
| V10 | E10 | combobox open 상태에서 문자 입력 | combobox filter 동작, typeahead 아님 | |
| V11 | P7 | findMatch 순수 함수 unit 테스트 | 버퍼+노드 리스트+현재 포커스 → 올바른 매칭 ID 반환 | |
