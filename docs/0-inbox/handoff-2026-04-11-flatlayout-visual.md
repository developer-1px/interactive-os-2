...
### 즉시 (다음 세션 첫 작업)
1. **디자인 토큰 수준 이슈 3건** — Skeleton contrast (sunken vs display 대비 약), Badge neutral tone action surface 색상, Meter/Progress 바 크기 (좁은 컬럼에서)
2. **개별 ui/ 컴포넌트 시각 완성** — Button, ListBox 등 핵심 컴포넌트에 tone/surface 다양화
3. **theme/showcase 페이지 통합** — NavNode/TabNode으로 3페이지를 하나로
