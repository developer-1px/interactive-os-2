...
graph TB
  ...
  subgraph L6["L6 · Primitives (React 접착 + Aria 컴포넌트)"]
    direction LR
  ...
  subgraph L5["L5 · Pattern (인터랙션 합성 = ARIA APG)"]
    direction TB
    subgraph L5_compose["합성"]
      compose["composePattern(config, ...axes) → AriaPattern"]
      ctx["createPatternContext — PatternContext 생성"]
    end
    subgraph L5_types["타입"]
      types["AriaPattern · NodeState · FocusStrategy"]
    end
    subgraph L5_edit["편집 축"]
      edit["edit axis — 인라인 편집 keyMap"]
    end
  ...
  subgraph L4["L4 · Axis (읽기 전용 축 7종)"]
    direction TB
    subgraph L4_axes["7축 (Axis → partial keyMap + config)"]
      direction LR
    ...
    subgraph L4_types["타입"]
      ax_types["PatternContext · KeyMap · AxisConfig"]
    end
  end
  ...
  subgraph L3["L3 · Plugin (데이터 조작 표준)"]
    ...
    subgraph L3_enhance["보강"]
      ...
      cmd_spatial["spatial: spatialCommands · findNearest · findAdjacentGroup · spatialReachable · useSpatialNav"]
...
| 색상 | 레이어 | 디렉토리 | 역할 | 렌더러 독립 |
|------|--------|----------|------|------------|
| 🔵 | L1 Store | `store/` | 데이터 구조 + CRUD | ✅ |
| 🔵 | L2 Engine | `engine/` | 커맨드 디스패치 + 미들웨어 | ✅ |
| 🟢 | L3 Plugin | `plugins/` | 데이터 조작 표준 (definePlugin) | ✅ |
| 🟢 | L4 Axis | `axis/` | 읽기 전용 축 7종 (keyMap + config) | ✅ |
| 🟢 | L5 Pattern | `pattern/` | 인터랙션 합성 (composePattern → AriaPattern) | ✅ |
| 🟠 | L6 Primitives | `primitives/` | Aria 컴포넌트 + hook (React 접착) | ❌ (React) |
| 🟠 | L7 UI | `ui/` | 표준 UI 완성품 (15종+) | ❌ (React) |
| 🔴 | 각 레이어 GAP | — | 안개 영역 | — |

**의존 방향:** L7 → L6 → L5 → L4 → L3 → L2 → L1 (단방향, 하위 레이어는 상위를 모름)

## Key Renames (from previous architecture)

| Before | After | 비고 |
|--------|-------|------|
| `AriaBehavior` | `AriaPattern` | L5 Pattern 핵심 타입 |
