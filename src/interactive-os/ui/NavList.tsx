...
const defaultRenderGroupLabel = (label: string): React.ReactNode => (
  <div className={ax({ role: 'badge', surface: 'ghost', textStyle: 'overline', tone: 'neutral-dim', cs: 'xs' })}>{label}</div>
)
    ...
    <div {...(nav.rootProps as React.HTMLAttributes<HTMLDivElement>)} className={`${(nav.rootProps as React.HTMLAttributes<HTMLDivElement>).className ?? ''} ${ax({ layout: 'stack', gap: 'lg' })}`}>
      {rootChildren.map((id) => {
        ...
        if (isGroup(entity)) {
          ...
          return (
            <div key={id} role="group" aria-label={getLabel(entity)} className={`navlist-group ${ax({ layout: 'stack', gap: 'sm' })}`}>
