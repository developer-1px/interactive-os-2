  ...
  return (
    <div className={ax({ layout: 'stack' })}>
      <Section label="preset=doc (default) — filename + region landmark">
        <CodePreview code={code} filename="example.ts" preset="doc" />
      </Section>

      <Section label="preset=presentation — mac chrome + large font">
        <CodePreview code={code} filename="app.tsx" preset="presentation" />
      </Section>

      <Section label="preset=chat — compact + wrap + no line numbers">
        <CodePreview code={code} preset="chat" />
      </Section>

      <Section label="preset=replay — compact + line numbers, plain figure">
        <CodePreview code={longCode} filename="styles.css" preset="replay" highlightLines={mixed} />
      </Section>

      <Section label="consecutive deleted (stronger tone)">
        <CodePreview code={longCode} filename="styles.css" highlightLines={consecutiveDeleted} />
      </Section>

      <Section label="consecutive inserted">
        <CodePreview code={longCode} filename="styles.css" highlightLines={consecutive} />
      </Section>

      <Section label="mixed tones: inserted(1) / deleted(3) / edited(5) / selected(7)">
        <CodePreview code={longCode} filename="styles.css" highlightLines={mixed} />
      </Section>

      <Section label="startLine=42 — gutter starts at 42">
        <CodePreview code={code} filename="example.ts" startLine={42} />
      </Section>

      <Section label="no filename — aria-label='Code example, typescript'">
        <CodePreview code={code} preset="doc" />
      </Section>
    </div>
  )
}
