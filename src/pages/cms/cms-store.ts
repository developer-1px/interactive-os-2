...
export const cmsStore = createStore({
  entities: {
    ...
    // Tab 1: Overview (features variant — label + title + desc + 2 cards)
    ...
    'tab-1-label':      { id: 'tab-1-label',      data: { type: 'section-label', value: localeMap('Architecture') } },
    'tab-1-title':      { id: 'tab-1-title',      data: { type: 'section-title', value: localeMap('Headless by design') } },
    'tab-1-desc':       { id: 'tab-1-desc',       data: { type: 'section-desc', value: localeMap('State lives in a normalized store, mutations flow through commands, and behaviors produce ARIA attributes — no DOM coupling.') } },
    'tab-1-card-a':     { id: 'tab-1-card-a',     data: { type: 'card' } },
    'tab-1-card-a-icon': { id: 'tab-1-card-a-icon', data: { type: 'icon', value: 'database' } },
    'tab-1-card-a-title': { id: 'tab-1-card-a-title', data: { type: 'text', role: 'title', value: localeMap('Normalized Store') } },
    'tab-1-card-a-desc': { id: 'tab-1-card-a-desc', data: { type: 'text', role: 'desc', value: localeMap('Flat entities + relationships. O(1) lookups, immutable snapshots, parent-child traversal built in.') } },
    'tab-1-card-b':     { id: 'tab-1-card-b',     data: { type: 'card' } },
    'tab-1-card-b-icon': { id: 'tab-1-card-b-icon', data: { type: 'icon', value: 'cog' } },
    'tab-1-card-b-title': { id: 'tab-1-card-b-title', data: { type: 'text', role: 'title', value: localeMap('Command Engine') } },
    'tab-1-card-b-desc': { id: 'tab-1-card-b-desc', data: { type: 'text', role: 'desc', value: localeMap('Every mutation is a command with undo/redo. Middleware pipeline for validation and logging.') } },

    // Tab 2: Details (stats variant — 3 stat containers)
    ...
    'tab-2-stat-a':     { id: 'tab-2-stat-a',     data: { type: 'stat' } },
    'tab-2-stat-a-val': { id: 'tab-2-stat-a-val', data: { type: 'stat-value', value: '6' } },
    'tab-2-stat-a-lbl': { id: 'tab-2-stat-a-lbl', data: { type: 'text', role: 'stat-label', value: localeMap('Behavior Axes') } },
    'tab-2-stat-b':     { id: 'tab-2-stat-b',     data: { type: 'stat' } },
    'tab-2-stat-b-val': { id: 'tab-2-stat-b-val', data: { type: 'stat-value', value: '14' } },
    'tab-2-stat-b-lbl': { id: 'tab-2-stat-b-lbl', data: { type: 'text', role: 'stat-label', value: localeMap('APG Patterns') } },
    'tab-2-stat-c':     { id: 'tab-2-stat-c',     data: { type: 'stat' } },
    'tab-2-stat-c-val': { id: 'tab-2-stat-c-val', data: { type: 'stat-value', value: '0' } },
    'tab-2-stat-c-lbl': { id: 'tab-2-stat-c-lbl', data: { type: 'text', role: 'stat-label', value: localeMap('Runtime Deps') } },

    // Tab 3: Integration (workflow variant — label + title + desc + 2 steps)
    'tab-3':            { id: 'tab-3',            data: { type: 'tab-item', label: localeMap('Integration') } },
    'tab-3-panel':      { id: 'tab-3-panel',      data: { type: 'tab-panel' } },
    'tab-3-section':    { id: 'tab-3-section',    data: { type: 'section', variant: 'workflow' } },
    'tab-3-label':      { id: 'tab-3-label',      data: { type: 'section-label', value: localeMap('Setup') } },
    'tab-3-title':      { id: 'tab-3-title',      data: { type: 'section-title', value: localeMap('Get started in two steps') } },
    'tab-3-desc':       { id: 'tab-3-desc',       data: { type: 'section-desc', value: localeMap('Pick a behavior, wire the headless state to your components, done.') } },
    'tab-3-step-1':     { id: 'tab-3-step-1',     data: { type: 'step' } },
    'tab-3-step-1-num': { id: 'tab-3-step-1-num', data: { type: 'step-num', value: '01' } },
    'tab-3-step-1-title': { id: 'tab-3-step-1-title', data: { type: 'text', role: 'step-title', value: localeMap('Install & Configure') } },
    'tab-3-step-1-desc': { id: 'tab-3-step-1-desc', data: { type: 'text', role: 'step-desc', value: localeMap('Add the package, create a store, pick plugins for your use case.') } },
    'tab-3-step-2':     { id: 'tab-3-step-2',     data: { type: 'step' } },
    'tab-3-step-2-num': { id: 'tab-3-step-2-num', data: { type: 'step-num', value: '02' } },
    'tab-3-step-2-title': { id: 'tab-3-step-2-title', data: { type: 'text', role: 'step-title', value: localeMap('Render & Ship') } },
    'tab-3-step-2-desc': { id: 'tab-3-step-2-desc', data: { type: 'text', role: 'step-desc', value: localeMap('Spread ARIA props onto your components. Keyboard interaction works out of the box.') } },
  ...
  relationships: {
    [ROOT_ID]: ['hero', 'stats', 'features', 'tab-group-1', 'workflow', 'patterns', 'footer'],
    ...
    'tab-1-section': ['tab-1-label', 'tab-1-title', 'tab-1-desc', 'tab-1-card-a', 'tab-1-card-b'],
    'tab-1-card-a': ['tab-1-card-a-icon', 'tab-1-card-a-title', 'tab-1-card-a-desc'],
    'tab-1-card-b': ['tab-1-card-b-icon', 'tab-1-card-b-title', 'tab-1-card-b-desc'],
    'tab-2': ['tab-2-panel'],
    'tab-2-panel': ['tab-2-section'],
    'tab-2-section': ['tab-2-stat-a', 'tab-2-stat-b', 'tab-2-stat-c'],
    'tab-2-stat-a': ['tab-2-stat-a-val', 'tab-2-stat-a-lbl'],
    'tab-2-stat-b': ['tab-2-stat-b-val', 'tab-2-stat-b-lbl'],
    'tab-2-stat-c': ['tab-2-stat-c-val', 'tab-2-stat-c-lbl'],
    'tab-3': ['tab-3-panel'],
    'tab-3-panel': ['tab-3-section'],
    'tab-3-section': ['tab-3-label', 'tab-3-title', 'tab-3-desc', 'tab-3-step-1', 'tab-3-step-2'],
    'tab-3-step-1': ['tab-3-step-1-num', 'tab-3-step-1-title', 'tab-3-step-1-desc'],
    'tab-3-step-2': ['tab-3-step-2-num', 'tab-3-step-2-title', 'tab-3-step-2-desc'],
