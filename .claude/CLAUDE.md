...
- 타입 체크: `pnpm typecheck` 사용. `tsc --noEmit`은 루트 tsconfig의 `files: []` 때문에 아무것도 체크하지 않음.
...
- `docs/3-resources/` 파일명: `{순번}-[{태그}]{제목}.md` (예: `12-[i18n]i18nEditorFeatures.md`). 순번은 폴더 내 마지막 번호 + 1. 태그는 주제 분류(design, tooling, methodology, pattern, i18n 등).
- 테스트 실패 시 원복 정책:
