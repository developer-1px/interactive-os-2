---
description: 구현 완료 후 Area MDX 문서를 갱신하여 결과를 누적한다. retro 후 실행하여 ⬜→🟢 전환, 새 L3 MDX 생성, 구조적 빈칸 발견을 수행한다. "/area", "area 갱신", "area 업데이트", "누적해", "문서 갱신" 등을 말할 때 사용. retro 완료 후 자동으로 제안할 수도 있다.
---

## /area — Area MDX 누적

> **파이프라인 위치**: `Discussion → PRD → Plan → 실행 → retro(갭 감지) → /area(누적)`
>
> retro가 "PRD vs 구현"을 비교하여 갭을 감지하는 역할이라면, /area는 "구현 결과를 Area MDX에 누적"하는 역할이다. 방향이 다르다: retro는 역방향(PRD ← 구현), area는 순방향(구현 → Area).
