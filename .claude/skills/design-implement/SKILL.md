...
## Step 0: Design Brief — 코드 전 시각적 판단

**CSS/JSX를 한 줄이라도 쓰기 전에**, `DESIGN_BRIEF.md` 템플릿의 빈칸을 채운다.
새 컴포넌트나 페이지를 만들 때는 10개 섹션 전부, 기존 수정은 관련 섹션만.

필수 판단 (빈칸을 못 채우면 코드 진행 금지):
1. **주인공 선언** — 이 화면의 hero 요소는? 나머지와 size 비율 2×+인가?
2. **깊이 지도** — 각 영역의 surface 레벨은? 자식 > 부모인가?
3. **정보 위계** — type 레벨 3종류 이하인가? color로 역할 구분했는가?
4. **밀도 선택** — compact/comfortable/spacious 중 하나 + 이유
5. **색 예산** — accent 2곳 이하, 무채색 90%+인가?
6. **인터랙션 맵** — hover/focus/selected에서 각각 무엇이 변하는가?

Brief를 채운 후에 아래 번들 체크리스트로 넘어간다.
